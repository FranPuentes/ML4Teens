...
class ImageSegmentAll(Block):
      ...
      def slot_not(self, slot, data):
          results = self._model(data, stream=False, verbose=False, visualize=False, **self._params);
          for r in results:
              if self.signal_mask():
      ...
      #-------------------------------------------------------------------------
      def run(self, **kwarg):
          raise RuntimeError("No tiene sentido invocar el método 'run' de un objeto de clase 'ImageSegmentAll'.");
