...
class Clustering(Block):
      ...
      def slot_label(self, slot, data):
          if data is not None:
             ...
             if   self.params.method.lower()=="pca":
                  from sklearn.decomposition import PCA;
                  pca = PCA(n_components=2);
                  df = pca.fit_transform(df);
             elif self.params.method is None or self.params.method.lower()=="t-sne" or self.params.method.lower()=="tsne":
             ...
             if   self.params.algorithm is None or self.params.algorithm.lower()=="kmeans":
                  ...
                  kmeans = KMeans(n_clusters=self.params.clusters or 2, **self.params.args);
                  clusters = kmeans.fit_predict(df);
             elif self.params.algorithm.lower()=="dbscan":
                  from sklearn.cluster import DBSCAN
                  dbscan = DBSCAN(n_clusters=self.params.clusters or 2, **self.params.args);
                  clusters = dbscan.fit_predict(df);
             elif self.params.algorithm.lower()=="agglomerative":
                  from sklearn.cluster import AgglomerativeClustering;
                  agglomerative = AgglomerativeClustering(n_clusters=self.params.clusters or 2, **self.params.args);
                  clusters = agglomerative.fit_predict(df);
             elif self.params.algorithm.lower()=="spectral":
                  from sklearn.cluster import SpectralClustering;
                  spectral = SpectralClustering(n_clusters=self.params.clusters or 2, **self.params.args);
                  clusters = spectral.fit_predict(df);             
             elif self.params.algorithm.lower()=="gaussian" or self.params.algorithm.lower()=="gmm":
                  from sklearn.mixture import GaussianMixture;
                  gmm = GaussianMixture(n_clusters=self.params.clusters or 2, **self.params.args);
                  gmm.fit(df);
             ...
             try:
               ...
               buf.seek(0);
               self.signal_image(PIL.Image.open(buf));
             finally:  
               buf.close();
